...
app.post('/api/seed', async (req, res) => {
  try {
    ...
    // Lessons (Uzbek)
    const lessons = [
      { id: 'lesson_1', title: "LEDni Yondirish", description: "Arduino yordamida LEDni yondirishni o'rganing!", level: 1, order: 1, components: ['Arduino Uno','LED','220\u03A9','Breadboard','Simlar'], codeExample: 'void setup(){pinMode(13,OUTPUT);}\nvoid loop(){digitalWrite(13,HIGH);delay(1000);digitalWrite(13,LOW);delay(1000);}', gameConfig: { gameType: 'energy_city', title: "Energy City - Chiroqlar Yonadi!" }, winCondition: { type: 'led_blink_count', count: 5 }, xpReward: 50, published: true },
      { id: 'lesson_2', title: "Svetafor Boshqaruvi", description: "3 xil LED bilan svetafor yasang!", level: 1, order: 2, components: ['Arduino Uno','3xLED','3x220\u03A9','Tugma','Simlar'], codeExample: 'void setup(){pinMode(13,OUTPUT);pinMode(12,OUTPUT);pinMode(11,OUTPUT);pinMode(2,INPUT);Serial.begin(9600);}\nvoid loop(){digitalWrite(13,HIGH);delay(3000);digitalWrite(13,LOW);digitalWrite(12,HIGH);delay(1000);digitalWrite(12,LOW);digitalWrite(11,HIGH);delay(3000);digitalWrite(11,LOW);}', gameConfig: { gameType: 'traffic_light', title: "Svetafor Challenge" }, winCondition: { type: 'cycles_completed', count: 10 }, xpReward: 75, published: true },
