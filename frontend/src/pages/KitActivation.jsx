...
export default function KitActivation() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setActivated(true);
      toast.success(`Kit activated! +${data.xpEarned} XP`);
      await fetchUser();
    } catch (err) {
    ...
    <div className="min-h-screen bg-dark-900 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-game text-white">Activate Your Kit</h1>
          <p className="text-dark-400 mt-2">Enter the activation code from your STEMVERSE kit</p>
        </div>
        {activated ? (
          <div className="card-glow text-center p-8">
            ...
            <h2 className="text-2xl font-game text-white mb-4">Kit Activated! 🎉</h2>
            <div className="flex gap-3 justify-center">
              <button onClick={() => navigate('/dashboard')} className="btn-primary">Dashboard</button>
              <button onClick={() => navigate('/lessons')} className="btn-secondary">Lessons</button>
            </div>
          </div>
        ) : (
          <div className="card-glow">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-dark-300 mb-2">Activation Code</label>
                ...
                <p className="text-xs text-dark-500 mt-2">Found inside your STEMVERSE kit box</p>
              </div>
            ...
            <div className="mt-6">
              <button onClick={() => navigate('/dashboard')} className="inline-flex items-center gap-2 text-dark-400 hover:text-white text-sm"><ArrowLeft className="w-4 h-4" /> Skip</button>
            </div>
          </div>
        )}
      </div>
