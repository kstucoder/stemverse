    ...
    <div className="min-h-screen bg-game-gradient">
        ...
        <nav className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Link to="/auth/login" className="btn-secondary text-sm px-5 py-2">Sign In</Link>
              <Link to="/auth/register" className="btn-primary text-sm px-5 py-2">Get Started</Link>
            </div>
          </div>
        </nav>
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-32">
          <div className="text-center animate-fade-in">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-500/10 border border-brand-500/20 mb-6">
              ...
              <span className="text-sm text-brand-300 font-semibold">Digital Twin Learning Platform</span>
            </div>
            <h1 className="text-4xl sm:text-6xl md:text-7xl font-game text-white leading-tight mb-6">
              Build Real Circuits.<br /><span className="text-gradient">Play Virtual Games.</span>
            </h1>
            <p className="text-lg md:text-xl text-dark-300 max-w-2xl mx-auto mb-10">
              Learn electronics and programming with Arduino. Connect real hardware to interactive games!
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link to="/auth/register" className="btn-primary text-lg px-8 py-4 flex items-center gap-2">
                Start Learning <ArrowRight className="w-5 h-5" />
              </Link>
              <Link to="/auth/login" className="btn-secondary text-lg px-8 py-4">I have an account</Link>
            </div>
          </div>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-game text-white mb-4">How It <span className="text-gradient">Works</span></h2>
        </div>
        <div className="grid md:grid-cols-4 gap-6">
          {[
            { icon: Cpu, title: '1. Build Circuit', desc: 'Connect components on your breadboard' },
            { icon: Zap, title: '2. Write Code', desc: 'Program your Arduino' },
            { icon: Gamepad2, title: '3. Connect & Play', desc: 'See your hardware in the game!' },
            { icon: Trophy, title: '4. Earn Rewards', desc: 'Complete lessons, earn XP' },
          ].map((f, i) => (
      ...
      <footer className="border-t border-dark-800 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-dark-500 text-sm">Digital Twin Learning Platform</p>
        </div>
      </footer>
    </div>
  );
