...
export default function ClassroomView() {
  ...
  useEffect(() => {
    teacherAPI.getClassroom(id).then(r => setClassroom(r.data)).catch(() => toast.error('Sinf topilmadi')).finally(() => setLoading(false));
  }, [id]);
  ...
  if (!classroom) return <div className="min-h-screen bg-dark-900 flex items-center justify-center"><p className="text-dark-400">Sinf topilmadi</p></div>;
    ...
    <div className="min-h-screen bg-dark-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="badge-level text-xs">{students.length} o'quvchi</span>
            </div>
            <p className="text-sm text-dark-400 mt-1">Taklif kodi: <span className="font-mono text-brand-400 cursor-pointer" onClick={() => { navigator.clipboard.writeText(classroom.inviteCode); toast.success('Nusxalandi!'); }}>{classroom.inviteCode} <Copy className="w-3 h-3 inline" /></span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="card-glow text-center"><TrendingUp className="w-5 h-5 text-neon-cyan mx-auto mb-2" /><p className="text-2xl font-game text-white">{totalXP}</p><p className="text-xs text-dark-400">Umumiy XP</p></div>
          <div className="card-glow text-center"><Trophy className="w-5 h-5 text-neon-yellow mx-auto mb-2" /><p className="text-2xl font-game text-white">{avgLevel.toFixed(1)}</p><p className="text-xs text-dark-400">O'rtacha Level</p></div>
        </div>

        <div className="card-glow">
          <h3 className="font-game text-white mb-4 flex items-center gap-2"><Users className="w-5 h-5 text-brand-400" /> O'quvchilar</h3>
          {students.length === 0 ? (
            <p className="text-dark-400 text-sm">Hali o'quvchilar yo'q. Taklif kodini o'quvchilaringiz bilan ulashing!</p>
          ) : (
