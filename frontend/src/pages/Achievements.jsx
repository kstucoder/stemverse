  ...
  return (
    <div className="min-h-screen bg-dark-900">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-game text-white flex items-center gap-3"><Trophy className="w-8 h-8 text-neon-yellow" /> Yutuqlar</h1>
        </div>
        <div className="card-glow mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-game text-white">{earned}/{items.length}</p><p className="text-xs text-dark-400">Qo'lga kiritilgan</p></div>
          </div>
          <div className="w-32">
          ...
          {items.map((a) => (
            <div key={a.id} className={`rounded-2xl border p-5 text-center ${a.earned ? 'bg-dark-800/80 border-neon-yellow/30' : 'bg-dark-800/30 border-dark-700 opacity-60'}`}>
              ...
              <div className="flex items-center justify-center gap-2 mt-4">
                {a.earned ? <><CheckCircle className="w-4 h-4 text-neon-green" /><span className="badge-completed text-xs">Qo'lga kiritilgan!</span></> : <><Lock className="w-4 h-4 text-dark-500" /><span className="badge-locked text-xs">Yopiq</span></>}
        ...
        {items.length === 0 && (
          <div className="text-center py-16"><Sparkles className="w-12 h-12 text-dark-500 mx-auto mb-4" /><h3 className="text-xl font-game text-dark-400">Hali yutuqlar yo'q</h3></div>
        )}
      </div>
    </div>
  );
