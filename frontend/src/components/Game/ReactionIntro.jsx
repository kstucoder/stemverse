...
//   1) Tinch tungi shahar (establishing shot) — letterbox, vignette, film-grain.
...
//   4) ZARBA: oq flash, shockwave halqalari, chang buluti, uchib chiqqan debris,
//      krater, kuchli silkinish, shahar bir lahza qizil-olovda.
//   5) XAVFLI MODDA chaqmoqday kuch bilan yer qa'riga — juda chuqurga kirib ketadi:
//      yashil-oq elektr nayzasi, yoriq tomirlari yon tomonga tarqaydi, kristallar
//      zaharli yashilda yonadi, chuqurlikda pulslanuvchi shu'la qoladi.
//   6) Electra missiyani aytadi.
import { useMemo, useRef, useState } from 'react';
import { Container, Graphics, Sprite, Texture, TilingSprite } from 'pixi.js';
import { AdvancedBloomFilter } from 'pixi-filters';
import PixiStage from './pixi/PixiStage';
import { gradTexture, radialTexture, makeSkyline } from './pixi/cityScene';
import DialogueBox from './DialogueBox';
import useGameStore from '../../stores/gameStore';
import { playThunder, playRift, playZap, playWhoosh, playBoom } from './gameAudio';
...
const T = { LETTER: 0.9, ENTRY: 1.6, DESCENT: 2.3, IMPACT: 4.15, SUBST: 5.15, END: 8.2 };
...
function makeFX(parent, tex) {
  ...
  return {
    ...
    clear() { pool.forEach(p => p.s.destroy()); pool.length = 0; },
  };
}
...
function drawBolt(g, ytop, ybot, jitter) {
  ...
  const stroke = (w, col, a) => { g.moveTo(pts[0][0], pts[0][1]); for (let i = 1; i < pts.length; i++) g.lineTo(pts[i][0], pts[i][1]); g.stroke({ width: w, color: col, alpha: a }); };
  stroke(9, 0x39e06a, 0.35);   // tashqi yashil halo
  stroke(5, 0x9dff6a, 0.85);   // yashil yadro
  stroke(1.8, 0xf4ffe6, 0.95); // oq-issiq markaz
...
function buildIntroScene(app, ctlRef, onSceneDone) {
  ...
  const moonGlow = new Sprite(radialTexture('rgba(180,205,255,0.18)', 512)); moonGlow.anchor.set(0.5); moonGlow.width = moonGlow.height = 320; moonGlow.x = 200; moonGlow.y = 92; world.addChild(moonGlow);
  const moon = new Sprite(radialTexture('rgba(210,222,255,0.6)', 256)); moon.anchor.set(0.5); moon.width = moon.height = 108; moon.x = 200; moon.y = 92; world.addChild(moon);
  const starC = new Container(); world.addChild(starC); const stars = [];
  for (let i = 0; i < 90; i++) { const g = new Graphics().circle(0, 0, 0.6 + Math.random() * 1.4).fill(0xeaf3ff); g.x = Math.random() * LW; g.y = Math.random() * (SURF - 20); starC.addChild(g); stars.push({ g, b: 0.3 + Math.random() * 0.55, sp: 0.5 + Math.random() * 2, ph: Math.random() * 6.28 }); }

  // ogohlantiruvchi qizil shu'la (osmon yorishadi)
  ...
  // ------- shahar silueti (ko'p qatlam, chuqurlik hissi) -------
  const cityGlow = new Sprite(radialTexture('rgba(70,110,210,0.16)', 512)); cityGlow.anchor.set(0.5); cityGlow.width = LW * 1.3; cityGlow.height = 260; cityGlow.x = LW / 2; cityGlow.y = SURF + 6; world.addChild(cityGlow);
  const city1 = makeSkyline(120, 0x0b1428, 23); city1.y = SURF - 470; city1.alpha = 0.5; world.addChild(city1);
  const city2 = makeSkyline(80, 0x0f1c38, 17); city2.y = SURF - 470; city2.alpha = 0.85; world.addChild(city2);
  // shahar derazalari (miltillovchi) — zarbada olovda yonadi
  const winC = new Container(); world.addChild(winC); const wins = [];
  for (let i = 0; i < 46; i++) { const g = new Graphics().rect(0, 0, 3.4, 5).fill(0xffd76a); g.x = 40 + Math.random() * 920; g.y = SURF - 8 - Math.random() * 130; g.alpha = 0.15 + Math.random() * 0.4; winC.addChild(g); wins.push({ g, b: g.alpha, sp: 0.5 + Math.random() * 2, ph: Math.random() * 6.28 }); }
  ...
  // kristall qatlamdagi gavharlar (modda yetganda yashil yonadi)
  const gems = [];
  for (let i = 0; i < 14; i++) { const g = new Graphics().poly([0, -6, 4, 0, 0, 6, -4, 0]).fill(0x2e4a44); g.x = 70 + Math.random() * 860; g.y = SURF + 195 + Math.random() * 70; g.alpha = 0.5; world.addChild(g); gems.push({ g, x: g.x, y: g.y }); }

  // chuqurlik shu'lasi + yoriq + tomirlar + nayza
  ...
  // yoriq tomirlari (shox-shabba) — modda kirganda tarqaydi
  const veins = [];
  ...
  const mCore = new Graphics(); // toshli yadro (notekis ko'pburchak)
  ...
  const smoke = makeFX(world, softTex);   // chang/tutun
  const embers = makeFX(world, softTex);  // olovli embers
  const debris = [];                      // uchib chiqqan tosh bo'laklari

  // ------- ekran-fazoviy qatlamlar (kadr ustida) -------
  ...
  const rings = []; // shockwave halqalari
  ...
  function doImpact() {
    ...
    // chang buluti (ko'tariluvchi, kengayuvchi)
    for (let i = 0; i < 22; i++) {
  ...
  app.ticker.add((tk) => {
    ...
    // descent davomida sekin push-in, zarbada bir turtki
    let zoom = 1;
    ...
    // ----- yulduz/deraza miltillashi -----
    stars.forEach(s => { s.g.alpha = s.b * (0.4 + 0.5 * Math.sin(t * s.sp + s.ph)); });
    wins.forEach(o => { o.g.alpha = o.b * (0.6 + 0.4 * Math.sin(t * o.sp + o.ph)) * (impacted ? clamp(1 - boltP, 0.15, 1) : 1); });
    ...
    if (t >= T.ENTRY && t < T.IMPACT && !impacted) {
      ...
      // iz embers (yo'lda uchqun sochadi)
    ...
    if (impacted && t >= T.SUBST) {
      ...
      veins.forEach(v => { if (v.rev <= boltP + 0.05) {
        const a = clamp((boltP - v.rev) * 3, 0, 1);
        veinG.moveTo(v.x1, v.y1).lineTo(v.x2, v.y2).stroke({ width: v.w, color: 0x7dff5a, alpha: a * pulse });
      }});
      // deep glow chuqurlashadi
      ...
      // kristallar yonadi
      gems.forEach(gm => { const on = clamp((boltP - (gm.y - SURF) / (LH - SURF)) * 3, 0, 1); gm.g.tint = 0xffffff; gm.g.alpha = 0.5 + on * 0.5; gm.g.scale.set(1 + on * 0.5); if (on > 0) gm.g.clear().poly([0, -6, 4, 0, 0, 6, -4, 0]).fill([0x8ff4c0, 0x9dff6a, 0x6bff8a][gm.x % 3 | 0]); });
      // yer titrashi + qa'r zarralari
      rumbleAcc += dt;
      if (substP < 1.4 && rumbleAcc > 0.42) { rumbleAcc = 0; playThunder(); useGameStore.getState().triggerShake(6); camShake = Math.max(camShake, 0.4); if (Math.random() < 0.6) playZap(); }
