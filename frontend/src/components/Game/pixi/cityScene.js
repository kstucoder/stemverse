// cityScene — Energy City olamining umumiy qurilish bloklari.
// Ham o'yin (EnergyCity.jsx), ham kirish sahnasi (EnergyCityIntro.jsx)
// AYNAN BITTA shahardan foydalanadi: bola cutscene'da ko'rgan shahar —
// o'yinda yoqadigan shaharning o'zi.
import { Container, Graphics, Sprite, Texture } from 'pixi.js';
import { AdvancedBloomFilter } from 'pixi-filters';
import { Eases, createTweens } from './tween';
...
export function gradTexture(stops, height = 512) {
...
export function radialTexture(color, size = 256) {
  const cv = document.createElement('canvas');
...
/* ---------- yomg'ir (bo'ron sahnasi uchun) ---------- */

export function makeRain(parent, count = 150) {
  const drops = [];
  const c = new Container();
  parent.addChild(c);
  for (let i = 0; i < count; i++) {
    const g = new Graphics().moveTo(0, 0).lineTo(-2.5, 11).stroke({ width: 1, color: 0x9ecbff, alpha: 0.5 });
    g.x = Math.random() * (LW + 200) - 100;
    g.y = Math.random() * LH;
    c.addChild(g);
    drops.push({ g, sp: 480 + Math.random() * 320 });
  }
  let intensity = 0;
  return {
    set(v) { intensity = v; },
    tick(dt) {
      ...
      drops.forEach((d) => {
        d.g.y += d.sp * dt;
        d.g.x -= d.sp * 0.18 * dt;
        if (d.g.y > LH) { d.g.y = -14; d.g.x = Math.random() * (LW + 200) - 60; }
      });
    },
  };
}
...
export function makeBuildingSpecs() {
...
export function makeBuilding(spec, tweens, particles) {
  const c = new Container();
  c.x = spec.x;
  c.y = GROUND_Y;
  ...
  return {
    ...
    spec,
    get lit() { return lit; },
    // instant=true → animatsiyasiz darhol yonadi/o'chadi (intro boshlanishi uchun)
    setLit(v, instant = false) {
      if (v === lit) return false;
      lit = v;
      if (instant) {
        trim.alpha = v ? 1 : 0.05;
        wins.forEach((wn) => { wn.on = v ? 1 : 0; });
        return v;
      }
      if (v) {
        ...
        if (particles) particles.burst(spec.x, GROUND_Y - spec.h, 0xffd700, 16, 150);
      } else {
        tweens.add({ duration: 0.35, update: (k) => {
...
export function makeSkyline(specsY, color, seed) {
...
export function makeWires(specs, parent) {
...
/* ---------- to'liq shahar yig'ish ---------- */

export function assembleCity(app, { startLit = false } = {}) {
  ...
  if (startLit) {
    buildings.forEach((b, i) => { b.setLit(true, true); pools[i].alpha = 0.4; });
  }

  return {
    app, tweens, particles,
    skyC, skyNight, skyDay, starC, stars, moon, sun, shoot, fogA, fogB,
    root, specs, buildings, pools, wires, lamps, tram,
    // ichki silliqlangan holatlar
    dayAlpha: 0, energyS: startLit ? 85 : 0, tramSpeed: 22, shootTimer: 5, shootLife: 99,
  };
}

// Har frame: layout + ambient + ctl bo'yicha shahar holati.
// ctl = { litCount, energy, night, tramOn, shooting, instantLit, onNewLight }
export function cityTick(city, dt, t, ctl) {
  const { app, root, skyNight, skyDay, starC, stars, moon, sun, shoot, fogA, fogB, buildings, pools, lamps, tram, wires } = city;
  ...
  skyDay.alpha = city.dayAlpha;
  moon.alpha = 1 - city.dayAlpha;
  sun.alpha = city.dayAlpha;
  starC.alpha = 1 - city.dayAlpha;
  ...
  // uchar yulduz (bo'ronda o'chirib qo'yiladi)
  if (ctl.shooting !== false) {
    city.shootTimer -= dt;
    if (city.shootTimer <= 0) {
      city.shootTimer = 6 + Math.random() * 9;
      shoot.x = w * (0.1 + Math.random() * 0.5);
      shoot.y = 30 + Math.random() * 90;
      city.shootLife = 0;
    }
    city.shootLife += dt;
    if (city.shootLife < 0.9) {
      shoot.x += 500 * dt;
      shoot.y += 170 * dt;
      shoot.alpha = Math.max(0, 1 - city.shootLife / 0.9) * (1 - city.dayAlpha) * 0.9;
    } else shoot.alpha = 0;
  } else shoot.alpha = 0;
  ...
  buildings.forEach((b, i) => {
    const shouldLit = i < ctl.litCount;
    if (b.setLit(shouldLit, ctl.instantLit) && shouldLit && ctl.onNewLight) ctl.onNewLight(i);
    b.tick(t, i);
    pools[i].alpha += ((shouldLit ? 0.4 : 0) - pools[i].alpha) * Math.min(dt * 3, 1);
  });

  // energiya
  city.energyS += (ctl.energy - city.energyS) * Math.min(dt * 2.5, 1);
  lamps.forEach((l) => l.set(city.energyS / 100));
  wires.tick(dt, city.energyS);

  // tramvay
  city.tramSpeed += ((ctl.tramOn ? 150 : 22) - city.tramSpeed) * Math.min(dt * 2, 1);
  tram.c.x += city.tramSpeed * dt;
  if (tram.c.x > LW + 80) tram.c.x = -80;
  tram.glow.alpha = ctl.tramOn ? 0.45 + 0.3 * Math.sin(t * 6) : 0.1;
  tram.light.alpha = ctl.tramOn ? 0.3 : 0.06;
}
